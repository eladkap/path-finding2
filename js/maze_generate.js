async function generateMazeRecursivelyAux(grid, startRow, endRow, startCol, endCol) {
    ...
    for (let j = startCol; j <= endCol; j++) {
      if (grid.getVertex(divideRow, j).getVertexType() != START_VERTEX && grid.getVertex(divideRow, j).getVertexType() != END_VERTEX) {
    ...
    for (let i = startRow; i <= endRow; i++) {
      if (grid.getVertex(i, divideCol).getVertexType() != START_VERTEX && grid.getVertex(i, divideCol).getVertexType() != END_VERTEX) {
  ...
  async function createMazeFrame(grid) {
    for (i = 0; i < grid.getRows(); i++) {
      if (grid.getVertex(i, 0).getVertexType() != START_VERTEX && grid.getVertex(i, 0).getVertexType() != END_VERTEX) {
    ...
    for (j = 0; j < grid.getCols(); j++) {
      if (grid.getVertex(0, j).getVertexType() != START_VERTEX && grid.getVertex(0, j).getVertexType() != END_VERTEX) {
    ...
    for (i = 0; i < grid.getRows(); i++) {
      if (grid.getVertex(i, grid.getCols() - 1).getVertexType() != START_VERTEX && grid.getVertex(i, grid.getCols() - 1).getVertexType() != END_VERTEX) {
    ...
    for (j = 0; j < grid.getCols(); j++) {
      if (grid.getVertex(grid.getRows() - 1, j).getVertexType() != START_VERTEX && grid.getVertex(grid.getRows() - 1, j).getVertexType() != END_VERTEX) {
